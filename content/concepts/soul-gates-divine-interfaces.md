---
title: "Soul's Seven Gates as Structured Divine Interfaces"
tags:
  - concepts
  - taarav
  - knowledge-graph
primary_theme: "or-yashar-or-chozer"
---

# Soul's Seven Gates as Structured Divine Interfaces

---

## Overview

The seven sense-portals of the embodied soul are structured interfaces between the human person and supernal reality — not merely physical apertures but loci where divine light must be governed by judgment (mishpat) to function as channels rather than breaches.

### Hebrew Terms

- **שערים** (gates / portals)
- **שופטים ושוטרים** (judges and officers)
- **ז' שערים שבנפש** (seven soul-gates)

### Maamarim That Advance This Concept

- [[maamarim/maamar-3601330131]]

## Position in the Conceptual Chain

Or Yashar and Or Chozer are now mapped onto the physiology of the embodied soul: the seven gates require their own form of structured discernment to prevent the overflow of divine light from shattering the vessel, extending the hemshech's analysis into embodied divine service.

## Cross-References

- [[concepts/index|Concept Index]]
- [[themes/or-yashar-or-chozer|Or Yashar v'Or Chozer]]
- [[themes/nefesh-habehamit|Nefesh HaBehamit]]
